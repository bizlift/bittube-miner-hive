#!/usr/bin/env bash
	...
	stats=
			...
			if [[ $? -ne 0 || -z $stats_raw ]]; then
				...
				local gpus_disabled=
				(head -n 40 /var/log/miner/bittube/bittube.log | grep -q "WARNING: backend AMD (OpenCL) disabled") && #AMD disabled found
				(head -n 40 /var/log/miner/bittube/bittube.log | grep -q "WARNING: backend NVIDIA disabled") && #and nvidia disabled
				gpus_disabled=1
